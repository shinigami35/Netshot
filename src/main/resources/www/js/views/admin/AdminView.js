...
define([
    ...
    'models/user/UserSshCollection',
    'views/admin/AddUserDialog',
    'views/admin/AddUserSshDialog',
    'views/admin/EditUserDialog',
    'views/admin/DeleteUserDialog',
    'views/admin/DeleteUserSshDialog',
    ...
    'text!templates/admin/userssh.html',
    'text!templates/admin/driver.html'
             ...
             UserCollection, UserSshCollection, AddUserDialog, AddUserSshDialog, EditUserDialog, DeleteUserDialog,
             DeleteUserSshDialog, DeviceTypeCollection, RefreshedDeviceTypeCollection,
             adminTemplate, adminToolbarTemplate, domainRowTemplate,
             credentialsRowTemplate, userRowTemplate, userSshRowTemplate, driverRowTemplate) {
    ...
    return Backbone.View.extend({
        ...
        userSshTemplate: _.template(userSshRowTemplate),
        ...
        usersSsh: new UserSshCollection([]),
        ...
        render: function () {
            ...
            // TODO
            this.$('#nsadmin-adduser-ssh').button({
                'icons': {
                    'primary': "ui-icon-plusthick"
                }
            }).click(function () {
                var addUserSshDialog = new AddUserSshDialog({
                    onAdded: function () {
                        that.refreshUsersSsh();
                    }
                });
                return false;
            });
        ...
        refreshUsersSsh: function () {
            var that = this;
            this.usersSsh.fetch().done(function () {
                var $table = that.$('#nsadmin-users-ssh table tbody');
                $table.empty();
                that.usersSsh.each(function (u) {
                    var $row = $(that.userSshTemplate(u.toJSON())).appendTo($table);
                    $row.find('.delete').button({
                        'icons': {
                            'primary': "ui-icon-trash"
                        },
                        ...
                        e.preventDefault();
                        var id = $(this).data('id');
                        var deleteUserSshDialog = new DeleteUserSshDialog({
                            onDeleted: function () {
                                that.refreshUsersSsh();
                            },
                            model: that.usersSsh.get(id)
                        });
                    });
                });
            });
